  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {rows.length === 0 && <p className="text-sm text-slate-400">{d.common.empty}</p>}
        {rows.map((t) => (
          <div key={t.id} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 min-w-[160px]">
            ...
            <div className="mt-2 space-y-0.5">
              <div className="text-xs text-green-600">+{money(t.incomeCents)} <span className="text-slate-400">({t.incomeCount})</span></div>
              <div className="text-xs text-red-600">-{money(t.expenseCents)} <span className="text-slate-400">({t.expenseCount})</span></div>
            </div>
          </div>
        ))}
      </div>
