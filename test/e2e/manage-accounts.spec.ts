import { test, expect, type Page } from "@playwright/test";

test.describe.configure({ mode: "serial" });

const modal = (page: Page) => page.locator("div.fixed.inset-0");

async function clickModalOk(page: Page, text: string) {
  const ok = modal(page).getByRole("button", { name: text, exact: true });
  await expect(ok).toBeVisible();
  await ok.click();
}

test.describe("账户管理页按钮", () => {
  const stamp = Date.now();
  const name = `E2E账户${stamp}`;
  const edited = `E2E账户改${stamp}`;

  test("着陆：显示标题与新增按钮", async ({ page }) => {
    await page.goto("/accounts");
    await expect(page.locator("h1", { hasText: "账户" })).toBeVisible();
    await expect(page.getByRole("button", { name: "新增" })).toBeVisible();
  });

  test("新增账户：表单保存 + 确认框", async ({ page }) => {
    await page.goto("/accounts");
    await page.getByRole("button", { name: "新增" }).click();
    const form = page.locator("form");
    await form.locator("input").nth(0).fill(name);
    await form.locator("input").nth(1).fill("100.00");
    await form.locator("input").nth(3).fill("E2E 备注");
    await form.getByRole("button", { name: "保存", exact: true }).click();
    await expect(modal(page).getByRole("heading", { name: "新增账户" })).toBeVisible();
    await clickModalOk(page, "保存");
    await expect(page.getByText(name, { exact: true })).toBeVisible();
    await expect(page.getByText("E2E 备注", { exact: true })).toBeVisible();
  });

  test("编辑账户：改名 + 确认框", async ({ page }) => {
    await page.goto("/accounts");
    const card = page.locator("div.rounded-2xl", { hasText: name });
    await expect(card).toHaveCount(1);
    await card.getByRole("button", { name: "修改" }).click();
    const form = page.locator("form");
    await form.locator("input").nth(0).fill(edited);
    await form.getByRole("button", { name: "保存", exact: true }).click();
    await expect(modal(page).getByRole("heading", { name: "保存账户" })).toBeVisible();
    await clickModalOk(page, "保存");
    await expect(page.getByText(edited, { exact: true })).toBeVisible();
    await expect(page.getByText(name, { exact: true })).toHaveCount(0);
  });

  test("删除账户：确认框 + 列表移除", async ({ page }) => {
    await page.goto("/accounts");
    const card = page.locator("div.rounded-2xl", { hasText: edited });
    await expect(card).toHaveCount(1);
    await card.getByRole("button", { name: "删除" }).click();
    await expect(modal(page).getByRole("heading", { name: "删除账户" })).toBeVisible();
    await clickModalOk(page, "删除");
    await expect(page.getByText(edited, { exact: true })).toHaveCount(0);
  });
});
