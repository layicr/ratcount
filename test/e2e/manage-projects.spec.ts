...
test.describe("项目管理页按钮", () => {
  const stamp = Date.now();
  const name = `E2E项目${stamp}`;
  const edited = `E2E项目改${stamp}`;
  ...
  test("删除项目：确认框 + 列表移除", async ({ page }) => {
    await page.goto("/projects");
    const card = page.locator("div.rounded-2xl.border-slate-200", { hasText: edited });
    await expect(card).toHaveCount(1);
    await card.getByRole("button", { name: "删除" }).click();
    await expect(modal(page).getByRole("heading", { name: "删除项目" })).toBeVisible();
