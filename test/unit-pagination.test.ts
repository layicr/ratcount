...
test("枚举: 各领域枚举非空且角色/类型/动作/频率取值正确", () => {
  ...
  assert.strictEqual(accountTypes.length, 22); // 含 borrowed（民间借贷·借入，负债）+ social_security（社保·保障）+ consumption_insurance（消费型保险·保障）/ includes borrowed (loan borrowed, liability) + social_security (protection) + consumption_insurance (protection)
});
